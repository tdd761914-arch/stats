...
def server_url(port: int, token: str, method: str) -> str:
    return f"http://127.0.0.1:{port}/bot{token}/{method}"
...
def wait_for_server(process: subprocess.Popen, token: str, port: int) -> dict:
    deadline = time.monotonic() + STARTUP_TIMEOUT
    last = None
    while time.monotonic() < deadline:
        if process.poll() is not None:
            raise RuntimeError("TRBotApi process exited before its HTTP listener became ready")
        last = request_json(server_url(port, token, "getMe"), {})
        if last.get("ok"):
            return last
        time.sleep(0.5)
    raise TimeoutError(f"TRBotApi listener did not become ready: {last or 'no response'}")
...
def official_url(token: str, method: str) -> str:
    return f"{OFFICIAL_BASE}/bot{token}/test/{method}"
...
def benchmark_bot(
    ...
    official_get_me = run_series(official_url(token, "getMe"), {}, samples)
    trbotapi_get_me = run_series(server_url(port, token, "getMe"), {}, samples)
    official_chat_action = run_series(official_url(token, "sendChatAction"), chat_payload, 1)
    trbotapi_chat_action = run_series(server_url(port, token, "sendChatAction"), chat_payload, 1)
    return {
        "id": identifier,
        ...
        "memory_after_kib": memory_snapshot(process.pid),
    }
...
def main() -> int:
    ...
    processes: list[tuple[subprocess.Popen, tempfile.NamedTemporaryFile, str, int]] = []
    started_at = datetime.now(timezone.utc).isoformat().replace("+00:00", "Z")
    try:
        for index, token in enumerate(tokens):
            process, log = start_server(args.trbotapi_dir, token, api_id, api_hash, args.test_dc, 18081 + index)
            processes.append((process, log, token, 18081 + index))
        startup_memory = {}
        for process, _log, token, port in processes:
            wait_for_server(process, token, port)
            startup_memory[bot_id(token)] = memory_snapshot(process.pid)

        bots = {}
        for process, _log, token, port in processes:
            measurement = benchmark_bot(token, port, process, args.samples, chat_id)
            measurement["memory"] = {
                "rss_start_kib": startup_memory[measurement["id"]].get("rss_kib"),
                "rss_after_kib": measurement["memory_after_kib"].get("rss_kib"),
                "hwm_kib": measurement["memory_after_kib"].get("hwm_kib"),
                "pss_kib": measurement["memory_after_kib"].get("pss_kib"),
                "vmsize_kib": measurement["memory_after_kib"].get("vmsize_kib"),
                "threads": measurement["memory_after_kib"].get("threads"),
            }
            measurement.pop("memory_after_kib", None)
            bots[measurement["id"]] = measurement

        binary = args.trbotapi_dir / "target" / "release" / "trbotapi-server"
        result = {
            "schema": 1,
            "status": "ok",
            "generated_at": started_at,
            "git_sha": os.environ.get("GITHUB_SHA") or "local",
            "runner": {"os": platform.platform(), "python": platform.python_version()},
            "samples_per_method": args.samples,
            "test_dc": args.test_dc,
            "artifact": {"release_binary_bytes": binary.stat().st_size},
            "bots": bots,
        }
    finally:
        for process, log, _token, _port in processes:
